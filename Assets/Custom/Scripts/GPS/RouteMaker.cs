...
public class RouteMaker : MonoBehaviour
{
   public enum LineType
        ...
        public void StartRoute(ARLocation.Location dest)
        {
        ...
        void generateRoute(ARLocation.Location _){
            ...
            StartCoroutine(
                        loader.LoadRoute(
                            ...
                            new RouteWaypoint { Type = RouteWaypointType.Location, Location = getPlaceLocation() },
                            (err, res) =>
                            {
                                apiError = err;
        ...
        ARLocation.Location getPlaceLocation(){

            ARLocation.Location location = new ARLocation.Location();
            

            string NombreMonumento = gameManager.monumentoEscena.name;

            switch(NombreMonumento){
                case "IndiaCatalina":
                SetLocationLatLong(location, 10.42648, -75.54404);

                break;

                case "Botas":
                SetLocationLatLong(location, 10.42127, -75.53764);

                break;

                case "Castillo":
                 SetLocationLatLong(location, 10.42219, -75.53944);

                break;

                case "Torre":
                 SetLocationLatLong(location, 10.423076, -75.549143);

                break;

                case "Pegasos":
                 SetLocationLatLong(location, 10.42216, -75.54878);
